...
void CSlg2Doc::ReadDataFile(CString filename, COpenMeasDlg *dlg)
{
	...
	if( fh == NULL) {
		AfxMessageBox( _T("ќшибка при открытии файла измерени€."));
		return;
	}

	//определ€ем тип файла статистики (старый(ascii) или новый(bin))
	BOOL bNewOld;
	char buf_check[2];
    ...
    if( m_dy_t3 != NULL) { delete m_dy_t3; m_dy_t3 = NULL;}
		...
		//заказываем пам€ть под измерени€
